...
KINDS = {
    "adr": ("ADR", Path("40-records/adr"), "record"),
    "review": ("REVIEW", Path("40-records/reviews"), "record"),
    "postmortem": ("POSTMORTEM", Path("40-records/postmortems"), "record"),
    "lesson": ("LESSON", Path("40-records/lessons"), "record"),
    "workorder": ("WO", Path("40-records/workorders"), "record"),
    "debt": ("DEBT", Path("40-records/tech-debt"), "record"),
    "gate": ("GATE", Path("30-gates/gates"), "gate"),
    "qa": ("QA", Path("40-records/qa-plans"), "record"),
    "agent-feedback": ("AF", Path("50-agent/agent-feedback"), "record"),
}
...
def append_gate_index(gov_root: Path, record_id: str, title: str) -> None:
    index = gov_root / "30-gates" / "GATE-INDEX.md"
...
def main() -> int:
    ...
    if args.kind == "gate":
        print(f"updated: {gov_root / '30-gates' / 'GATE-INDEX.md'}")
