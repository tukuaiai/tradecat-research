...
def minimal_files() -> dict[str, str]:
    return {
        "README.md": doc(
            ...
            "工程治理包",
            """
本目录是项目级工程治理包，固定落点为 `assets/docs/governance/`。
...
1. 先读 `INDEX.md`。
2. 再读 `00-start-here/AGENT-ENTRY.md`。
3. 按 `00-start-here/CONTEXT-ROUTER.md` 选择最小上下文。
4. 需要模块事实时，通过 `00-start-here/CONTEXT-MAP.md` 找到对应 module context。
            """,
        ),
        "INDEX.md": doc(
            "GOV-INDEX",
...
- `00-start-here/AGENT-ENTRY.md`：代理启动协议。
- `00-start-here/CONTEXT-MAP.md`：项目上下文地图。
- `00-start-here/CONTEXT-ROUTER.md`：任务类型到上下文包的路由。
- `00-start-here/PROJECT-TOPOLOGY.md`：项目结构和边界说明。
...
- `30-gates/GATE-INDEX.md`
        ...
        "00-start-here/AGENT-ENTRY.md": doc(
            "GOV-AGENT-ENTRY",
            "process",
            "Agent Entry",
            """
...
2. 开始任务前先读取 `assets/docs/governance/INDEX.md`。
3. 根据 `assets/docs/governance/00-start-here/CONTEXT-ROUTER.md` 选择最小上下文。
4. 涉及架构边界时必须读取相关 ADR。
5. 涉及用户功能时必须产出 QA 计划或验证证据。
6. 高风险变更必须说明回滚路径。
7. 如果发现重复错误或标准缺失，记录到 `50-agent/agent-feedback/`。
            """,
        ),
        "00-start-here/CONTEXT-MAP.md": doc(
            "GOV-CONTEXT-MAP",
            "index",
            "Context Map",
            """
...
|---|---|---|---|---|
| 待补充 | `src/...` | `00-start-here/module-contexts/<module>/CONTEXT.md` | `40-records/adr/...` | `待补充` |

## 维护规则

- 不把模块上下文散落到代码目录。
- 模块上下文统一放在 `00-start-here/module-contexts/`。
- 原有模块 README 只被引用，不被治理包覆盖。
            """,
        ),
        "00-start-here/CONTEXT-ROUTER.md": doc(
            "GOV-CONTEXT-ROUTER",
            "process",
            "Context Router",
            """
...
1. `assets/docs/governance/INDEX.md`
2. `assets/docs/governance/00-start-here/PROJECT-TOPOLOGY.md`
3. `assets/docs/governance/00-start-here/CONTEXT-MAP.md`
...
| 复盘 | 文档治理规则、门禁与护栏 | postmortems/INDEX.md | 防复发动作 |
        ...
        "00-start-here/PROJECT-TOPOLOGY.md": doc(
...
6. 高风险变更说明回滚路径。
        ...
        "20-processes/RPI研究计划实施流程.md": doc(
            "PROC-RPI",
            "process",
            "RPI 研究-计划-实施流程",
            """
...
|---|---|---|---|---|---|
| 待补充 | BLOCK/WARN | 待补充 | manual/agent/test/CI | 待补充 | draft |
            """,
        ),
        "40-records/adr/INDEX.md": index_doc("ADR Index", []),
        "40-records/postmortems/INDEX.md": index_doc("Postmortem Index", []),
        "40-records/lessons/INDEX.md": index_doc("Lesson Index", []),
        "40-records/qa-plans/INDEX.md": index_doc("QA Plan Index", []),
        "50-agent/agent-feedback/INDEX.md": index_doc("Agent Feedback Index", []),
        "60-templates/ADR.template.md": template("ADR"),
        "60-templates/GATE.template.md": template("GATE"),
        "60-templates/QA.template.md": template("QA"),
        "60-templates/AGENT-FEEDBACK.template.md": template("AF"),
        "70-tools/README.md": doc(
            "TOOLS-GOVERNANCE",
            "tooling",
            "Governance Tools",
            """
...
- `rebuild_governance_index.py`：重建根索引、记录索引和 `GATE-INDEX.md`。
...
这些脚本只写入 `assets/docs/governance/` 内部；外部 CI、lint、hook 接入必须单独 opt-in。
            """,
        ),
        "80-runtime/README.md": doc(
            "RUNTIME-GOVERNANCE",
            "runtime",
            "Runtime Records",
            "这里放代理任务执行过程、运行记录和临时材料。长期价值内容应沉淀到 `40-records/`。",
        ),
        "90-archive/README.md": doc(
            "ARCHIVE-GOVERNANCE",
            "archive",
            "Archive",
            "这里归档失效、被替代或过期的治理资产。",
...
def full_extra_files() -> dict[str, str]:
    return {
        "10-standards/优质代码定义.md": doc(
            "STD-CODE-GOOD",
            "standard",
            "优质代码定义",
            "优质代码必须正确、清晰、边界明确、可测试、可维护、性能可解释，并优先复用成熟能力。",
        ),
        "10-standards/性能效率优化标准.md": doc(
            "STD-PERFORMANCE",
            "standard",
            "性能效率优化标准",
            "默认检查复杂度、hot path、数据库/API/I/O、缓存、内存、并发、成本、benchmark/profile 和 p95/p99。",
        ),
        "10-standards/可靠性标准.md": doc(
            "STD-RELIABILITY",
            "standard",
            "可靠性标准",
            "默认检查 timeout、retry budget、熔断、降级、幂等、背压、资源池、队列容量、恢复路径和可观测性。",
        ),
        "10-standards/架构设计原则.md": doc(
            "STD-ARCHITECTURE",
            "standard",
            "架构设计原则",
            "默认检查模块边界、依赖方向、单一真相源、信息隐藏、深模块、禁止旁路和数据所有权。",
        ),
        "10-standards/术语表.md": doc(
            "STD-GLOSSARY",
            "standard",
            "术语表",
            "记录项目领域词、模块名、缩写、状态名和关键概念。",
        ),
        "20-processes/代码评审标准.md": doc(
            "PROC-REVIEW",
            "process",
            "代码评审标准",
            "评审输出必须包含 PASS/WARN/BLOCK、证据、影响、最小修复和验证方式。",
        ),
        "20-processes/文档治理规则.md": doc(
            "PROC-DOC-GOVERNANCE",
            "process",
            "文档治理规则",
            "定义文档何时更新、如何归档、如何复核、如何从复盘生成护栏。",
        ),
        "40-records/reviews/INDEX.md": index_doc("Review Index", []),
        "40-records/workorders/INDEX.md": index_doc("Workorder Index", []),
        "40-records/tech-debt/INDEX.md": index_doc("Tech Debt Index", []),
        "50-agent/review-agents/INDEX.md": index_doc("Review Agents Index", []),
        "50-agent/prompts/INDEX.md": index_doc("Prompts Index", []),
        "50-agent/skills/INDEX.md": index_doc("Embedded Skills Index", []),
        "60-templates/REVIEW.template.md": template("REVIEW"),
        "60-templates/POSTMORTEM.template.md": template("POSTMORTEM"),
        "60-templates/LESSON.template.md": template("LESSON"),
        "60-templates/WORKORDER.template.md": template("WO"),
        "60-templates/DEBT.template.md": template("DEBT"),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Initialize an assets/docs/governance package.")
    ...
    parser.add_argument(
        "--no-embed-tools",
        action="store_true",
        help="Do not copy governance maintenance scripts into assets/docs/governance/70-tools/.",
    )
...
def embed_tools(root: Path, force: bool, dry_run: bool) -> tuple[list[str], list[str], list[str]]:
    ...
    for name in TOOL_NAMES:
        src = source_dir / name
        if not src.exists():
            skipped.append(str(GOV_ROOT / "70-tools" / name) + " (source missing)")
            continue
        dst = root / "70-tools" / name
        display = str(GOV_ROOT / "70-tools" / name)
...
def main() -> int:
    ...
    if skipped:
        ...
        for item in skipped:
            print(f"  = {item}")
    print("non_invasive: did not modify files outside assets/docs/governance/")
    return 0
