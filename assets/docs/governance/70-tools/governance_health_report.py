...
RECORD_DIRS = {
    "adr": ("ADR", Path("40-records/adr")),
    "review": ("REVIEW", Path("40-records/reviews")),
    "postmortem": ("POSTMORTEM", Path("40-records/postmortems")),
    "lesson": ("LESSON", Path("40-records/lessons")),
    "workorder": ("WO", Path("40-records/workorders")),
    "debt": ("DEBT", Path("40-records/tech-debt")),
    "gate": ("GATE", Path("30-gates/gates")),
    "qa": ("QA", Path("40-records/qa-plans")),
    "agent-feedback": ("AF", Path("50-agent/agent-feedback")),
}
...
def placeholder_docs(root: Path) -> list[str]:
    ...
    for path in markdown_files(root):
        ...
        if rel.parts and rel.parts[0] == "60-templates":
            continue
...
def stale_docs(root: Path, today: date) -> list[str]:
    ...
    for path in markdown_files(root):
        rel = path.relative_to(root)
        if rel.parts and rel.parts[0] in {"60-templates", "90-archive"}:
            continue
...
def active_feedback(root: Path) -> list[str]:
    directory = root / "50-agent" / "agent-feedback"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render an assets/docs/governance health report.")
