...
REQUIRED = [
    ...
    "40-records/qa-plans/INDEX.md",
    "50-agent/agent-feedback/INDEX.md",
    "60-templates/ADR.template.md",
    "60-templates/GATE.template.md",
    "60-templates/QA.template.md",
    "60-templates/AGENT-FEEDBACK.template.md",
    "70-tools/README.md",
    "80-runtime/README.md",
...
EMBEDDED_TOOLS = [
    "70-tools/init_governance_package.py",
    "70-tools/new_governance_record.py",
    "70-tools/new_module_context.py",
    "70-tools/rebuild_governance_index.py",
    "70-tools/validate_governance_package.py",
    "70-tools/governance_health_report.py",
    "70-tools/governance_context_bundle.py",
]
...
def check_duplicate_ids(ids: dict[str, list[str]], issues: list[dict[str, str]]) -> None:
    for doc_id, paths in sorted(ids.items()):
        if len(paths) > 1:
            issues.append({"severity": "WARN", "path": ", ".join(paths), "message": f"duplicate frontmatter id: {doc_id}"})
...
def validate(project_root: Path, strict: bool) -> dict[str, object]:
    ...
    for path in root.rglob("*.md"):
        ...
        if fm is None:
            ...
            if missing:
                issues.append({"severity": "WARN", "path": str(rel), "message": f"frontmatter missing: {', '.join(missing)}"})
            check_frontmatter_values(rel, fm, issues)
            check_markdown_links(project_root, root, rel, text, issues)
        if "/gates/" in f"/{rel.as_posix()}/" and path.name.startswith("GATE-"):
    ...
    if index.exists():
        ...
        for rel in ["00-start-here/AGENT-ENTRY.md", "30-gates/GATE-INDEX.md", "10-standards/工程质量标准.md"]:
            if rel not in content:
                issues.append({"severity": "WARN", "path": "INDEX.md", "message": f"index does not mention {rel}"})

    gate_index = root / "30-gates" / "GATE-INDEX.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate an assets/docs/governance package.")
